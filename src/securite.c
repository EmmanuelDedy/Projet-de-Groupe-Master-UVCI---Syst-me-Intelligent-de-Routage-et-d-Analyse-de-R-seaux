...
int dfs_cycle(int v, int visite[], int pile_rec[], Graphe *g) {
  ...
  while (courant != NULL) {
    ...
    if (g->noeuds[voisin].est_actif) {
      if (!visite[voisin]) {
        if (dfs_cycle(voisin, visite, pile_rec, g))
          return 1;
...
void dfs_points_critiques(int u, int visite[], int disc[], int low[],
                          int parent[], int est_articul[], Graphe *g,
                          int *temps) {
  int fils = 0;
  visite[u] = 1;
  disc[u] = low[u] = ++(*temps);
...
void identifier_points_critiques(Graphe *g) {
  ...
  for (int i = 0; i < g->nb_noeuds; i++) {
    if (g->noeuds[i].est_actif && !visite[i]) {
      dfs_points_critiques(i, visite, disc, low, parent, est_articul, g,
                           &temps);
    }
  }
  ...
  if (nb_critiques == 0)
    printf("Aucun point critique structurel détecté.\n");
...
// Wrapper global pour l'analyse de sécurité complète
void securite_analyser_vulnerabilites(Graphe *g) {
  ...
  // 1. Détection de cycles (boucles de routage)
  if (detecter_cycles(g)) {
    printf("[ATTENTION] Cycles de routage détectés dans le réseau.\n");
  } else {
    printf("[OK] Aucun cycle de routage détecté.\n");
  }

  // 2. Identification des points critiques (SPOF)
  identifier_points_critiques(g);

  // 3. Analyse du zonage (SCC)
