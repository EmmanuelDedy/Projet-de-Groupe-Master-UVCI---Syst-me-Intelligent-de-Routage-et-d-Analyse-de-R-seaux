...
Paquet* creer_paquet(int id, int priorite, float taille,
...
int enfiler(FileAttente* file, Paquet* nouveau_p) {
     if (!file || !nouveau_p) return -1;
    file->paquets_total++; 

    // On vérifie si la file est pleine
    if (file->capacite_max != -1 &&
        file->taille_actuelle >= file->capacite_max) {
...
void afficher_stats(FileAttente* file) {
    ...
    double temps_moyen = (file->paquets_defiles > 0) ? (file->somme_attente / file->paquets_defiles) : 0.0;

    // calcul du taux de pertes 
    double taux_perte = (file->paquets_total > 0) ? ((double)file->paquets_rejetes / file->paquets_total) * 100 : 0.0;

    // Calcul du débit effectif 
    double debit = (file->somme_attente > 0) ? ((double)file->paquets_defiles / file->somme_attente) : 0.0;
